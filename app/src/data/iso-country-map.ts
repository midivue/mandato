/**
 * ISO 3166-1 numeric → alpha-2 mapping for the world-atlas TopoJSON.
 * Only includes countries present in countries-110m.json.
 */
...
export const ALPHA2_TO_ISO_NUM: Record<string, string> = Object.fromEntries(
  Object.entries(ISO_NUM_TO_ALPHA2).map(([num, alpha]) => [alpha, num]),
)

export const EUROPEAN_ALPHA2 = new Set([
  'AL', 'AD', 'AT', 'BY', 'BE', 'BA', 'BG', 'HR', 'CY', 'CZ',
  'DK', 'EE', 'FI', 'FR', 'DE', 'GR', 'HU', 'IS', 'IE', 'IT',
  'XK', 'LV', 'LT', 'LU', 'MK', 'MT', 'MD', 'ME', 'NL', 'NO',
  'PL', 'PT', 'RO', 'RS', 'SK', 'SI', 'ES', 'SE', 'CH', 'UA',
  'GB', 'TR', 'GE',
])

export const EUROPEAN_ISO_NUMS = new Set(
  [...EUROPEAN_ALPHA2]
    .map((a) => ALPHA2_TO_ISO_NUM[a])
    .filter(Boolean),
)
