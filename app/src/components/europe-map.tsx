...
import { ISO_NUM_TO_ALPHA2, EUROPEAN_ISO_NUMS } from '@/data/iso-country-map'
import { MAP_COLOR_SCALE, getMapColor } from '@/lib/map-utils'
import europeTopo from '@/data/world-50m.json'
...
const allFeatures = (
  feature(
    europeTopo as unknown as Topology,
    (europeTopo as unknown as Topology).objects.countries,
  ) as FeatureCollection
).features

const europeFeatures = allFeatures.filter(
  (f) => f.id && EUROPEAN_ISO_NUMS.has(String(f.id)),
)
...
export function EuropeMap({ countryVotes }: EuropeMapProps) {
        ...
        {europeFeatures.map((f, idx) => {
          ...
          return (
            <path
              ...
              d={d}
              fill={isHU ? '#fef3c7' : getMapColor(countryVotes[alpha2] ?? 0, maxVotes)}
              stroke={isHU ? '#d97706' : '#fff'}
              strokeWidth={isHU ? 1.5 : 0.5}
